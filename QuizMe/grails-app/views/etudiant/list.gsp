			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="sexe" title="${message(code: 'etudiant.sexe.label', default: 'Sexe')}" />
					
						<g:sortableColumn property="email" title="${message(code: 'etudiant.email.label', default: 'Email')}" />
					
						<g:sortableColumn property="dateDeNaissance" title="${message(code: 'etudiant.dateDeNaissance.label', default: 'Date De Naissance')}" />
					
						<g:sortableColumn property="numEtudiant" title="${message(code: 'etudiant.numEtudiant.label', default: 'Num Etudiant')}" />
				...
				<g:each in="${etudiantInstanceList}" status="i" var="etudiantInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${etudiantInstance.id}">${fieldValue(bean: etudiantInstance, field: "prenom")}</g:link></td>
					
						<td>${fieldValue(bean: etudiantInstance, field: "nom")}</td>
					
						<td>${fieldValue(bean: etudiantInstance, field: "sexe")}</td>
					
						<td>${fieldValue(bean: etudiantInstance, field: "email")}</td>
					
						<td><g:formatDate date="${etudiantInstance.dateDeNaissance}" /></td>
					
						<td>${fieldValue(bean: etudiantInstance, field: "numEtudiant")}</td>
